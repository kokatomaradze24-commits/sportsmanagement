...
export type NotificationKind = Kind;
...
function buildText(kind: Kind, lang: Lang, v: TemplateVars): string {
  ...
  if (lang === "ka") {
    if (kind === "registration") return `გამარჯობა! ${v.playerName} წარმატებით დარეგისტრირდა ${club}-ში${sportPart}.`;
    if (kind === "schedule") return `${v.playerName}-ის გადახდის გრაფიკი ${club}-ში: ${v.months} თვე, ყოველთვიური თანხა ${v.amount}₾. პირველი ვადა: ${v.firstDueDate}.`;
...
export const sendEventSmsFn = createServerFn({ method: "POST" })
  .inputValidator((input: SendInput) => input)
  .handler(async ({ data }) => {
    const { token, userId, playerId, kind, paymentId } = data;
    if (!token || !userId || !playerId || !kind) {
    ...
    const lang: Lang = (data.lang && MONTHS_LONG[data.lang] ? data.lang : "en");
    ...
    if (settings.provider === "magti") {
      ...
      return { ok: true, skipped: "provider_not_sms" };
    }
...
/**
 * Fire-and-forget client helper. Failures are logged, never thrown.
 */
